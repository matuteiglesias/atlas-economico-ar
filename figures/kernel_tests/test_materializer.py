...
class FigureKernelTests(unittest.TestCase):
    ...
    def test_materializes_exactly_41_contract_valid_artifacts(self):
        frame_ids = validate_contract.validate_reference_frames(
            validate_contract.load_yaml(FIGURES / "reference_frames.yaml")
        )
        with tempfile.TemporaryDirectory(dir=ROOT) as tmp:
            ...
            self.assertEqual(manifest["artifact_count"], 41)
            self.assertEqual(len(list((output / "svg").glob("*.svg"))), 41)
            self.assertEqual(len(list((output / "png").glob("*.png"))), 41)
