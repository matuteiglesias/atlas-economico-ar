...
3. **Figure grammar and materialization** — `figures/CONTRACT.md`, active `figures/specs/`, bindings/resolvers and `plot-artifacts/manifest.json` own the executable figure seam from PlotIntent to PlotArtifact.
...
9. **Frontend** — `web/` is a thin consumer of compiled read models. It must use compiled publication capabilities rather than recreating curation or question-publication rules.
