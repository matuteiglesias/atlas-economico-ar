#!/usr/bin/env python3
"""Offline integrity validation for the frozen eighteen-Series BCRA tranche."""
...
def main() -> int:
    ...
    if len(entries) != 18:
        raise ValidationError(f"BCRA tranche freeze requires exactly 18 Series, found {len(entries)}")
    if registry.get("provider", {}).get("id") != "bcra_monetarias_v4":
        raise ValidationError("BCRA batch provider mismatch")
    if len({entry["provider_series_id"] for entry in entries}) != 18:
        raise ValidationError("duplicate BCRA provider id")
    if len({entry["canonical_indicator_id"] for entry in entries}) != 18:
        raise ValidationError("BCRA tranche must map eighteen distinct CanonicalIndicators")
    ...
    print("PASS: validated 18 authentic BCRA Series captures offline.")
