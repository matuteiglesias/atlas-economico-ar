...
export default async function Home() {
  ...
  return <><SiteHeader /><main className="home-page"><section className="home-hero"><p className="eyebrow">Argentina / Economic Atlas</p><h1>Explore the questions shaping Argentina&apos;s economy.</h1><p>Move from public questions to the concepts, measurements, and chart ideas that help examine them.</p><div><Link className="primary-link" href="/topics/inflation">Start with inflation →</Link><Link href="/atlas">Browse the full atlas →</Link></div></section>
    ...
    <section className="home-areas"><header><p className="eyebrow">Six economic areas</p><h2>One connected reading structure</h2></header><div>{navigation.regions.map((region) => <Link href={region.href} key={region.href} className={region.populated ? "populated" : ""}><small>{region.populated ? "Now available" : "Structure ready"}</small><h3>{region.title}</h3><p>{region.dek}</p></Link>)}</div></section>
    <section className="home-editorial"><div><h2><CircleHelp /> Selected questions</h2><LinkList items={area.questions.slice(0, 6)} /></div><div><h2><BarChart3 /> Selected charts</h2><div className="chart-grid">{area.charts.slice(0, 3).map((chart) => <ChartCard key={chart.href} item={chart} />)}</div></div></section>
  </main></>;
}
